...
class SequencedAgent(ScriptedAgent):
    """Return scripted responses in order while retaining clone compatibility."""
...
class WorktreeTests(unittest.TestCase):
    def test_null_natural_audit_retries_same_draft_before_reauthoring(self) -> None:
        original = Path.cwd()
        with tempfile.TemporaryDirectory() as temporary:
            project = Path(temporary) / "natural_review_retry_problem"
            project.mkdir()
            try:
                os.chdir(project)
                ...
                reviewer = SequencedAgent([None, audit])
                runtime = Runtime(
                ...
                self.assertTrue(
                    (node_dir / "natural-review-null-v1-attempt-1.txt").is_file()
                )
