...
`HUMANIZE_WIKI_DIR`. `HUMANIZE_CANDIDATE_BASE_COMMIT` is the node's immutable proof-base commit,
so project wrappers can attribute diagnostics across the complete candidate history, including
accepted-child overlays and every RLCR round, rather than only the final commit. A repository that
needs a different comparator target for each generated lemma should use these values in its wrapper.
