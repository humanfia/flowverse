...
must not edit control state, report logs, leaderboard state, manifests, audit packets, or cursors.
A source lock permits only one Lane 1 owner for the same original workspace. Runtime-owned paths
and report-log metadata are checked against that state; an out-of-band replacement or edit fails
closed rather than becoming coordination evidence.
