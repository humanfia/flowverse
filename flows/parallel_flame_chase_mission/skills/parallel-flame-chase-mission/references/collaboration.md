...
The coordinator plans once, then arrives in a fresh session for every audit. Three lanes run
concurrently; each has actors A and B alternating in fresh sessions.
...
live file during a turn. Mission audit manifests include the same board. This local evaluator
protocol does not authorize a remote competition submission, release, deployment, purchase, or
message. Lane 1 remains the only source integrator.
...
checkpoint does not announce completion, trigger an audit, replace the final structured report,
or carry across a different mission generation.
