...
class MissionController:
    ...
    def _validate_loaded(self) -> None:
        ...
        for mission in cast("list[dict[str, Any]]", self.data["missions"]):
        ...
        if any(not isinstance(mission_id, str) for mission_id in mission_ids) or len(
            set(mission_ids)
        ) != len(mission_ids):
            raise ValueError("mission state contains missing or duplicate mission IDs")
        ...
        audits = self._audits()
        audit_ids = [audit.get("id") for audit in audits]
        if any(not isinstance(audit_id, str) for audit_id in audit_ids) or len(
            set(audit_ids)
        ) != len(audit_ids):
            raise ValueError("mission state contains missing or duplicate audit IDs")
        for audit in audits:
        ...
        if any(not isinstance(item_id, str) for item_id in queue_ids) or len(
            set(queue_ids)
        ) != len(queue_ids):
            raise ValueError("integration queue contains missing or duplicate IDs")
        for item in queue:
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return cast("dict[str, Any]", _copy(self.data))
    ...
    def _ordered(values: Sequence[object]) -> list[LaneName]:
        return [cast("LaneName", lane) for lane in LANES if lane in values]
    ...
    def trigger(
        ...
        *,
        ...
        requested = (
            list(cast("tuple[LaneName, ...]", LANES))
            if scope == "global"
            else self._ordered(targets)
        )
        if not requested:
    ...
    def observe(self, lane: LaneName, report: dict[str, Any]) -> None:
        ...
        mission["outcome"] = _copy(event)
        cast("list[dict[str, object]]", self.data["outcomes"]).append(_copy(event))
        self._transition(
            mission,
    ...
    def mark_quiesced(
        ...
        if audit is None or lane not in audit["targets"]:
            return
        cast("dict[str, object]", audit["quiesced"])[lane] = _copy(interruption)
        cast("dict[str, object]", audit["evidence"])[lane] = _copy(evidence)
    ...
    def decision_packet(
        ...
        active_missions = {
            lane: _copy(self._mission(current.get(lane)))
            for lane in cast("tuple[LaneName, ...]", LANES)
        }
        return {
            "version": 1,
            ...
            "integration_queue": _copy(self._queue()),
            "last_external_progress": _copy(self.data.get("last_external_progress")),
            "latest_reports": _copy(latest_reports),
            "recent_audits": _copy(self._audits()[-10:]),
    ...
    def validate_decision(self, decision: AuditDecision) -> str | None:
        ...
        if decision.audit_id != audit["id"]:
            return f"audit_id must be {audit['id']}"
        if decision.revision != audit["revision"]:
            return f"revision must be {audit['revision']}"
        targets = cast("list[LaneName]", audit["targets"])
        if {item.lane for item in decision.lanes} != set(targets) or len(
            decision.lanes
        ) != len(targets):
        ...
        for item in decision.lanes:
            ...
            if (
                ...
                return (
                    f"{item.lane} accept requires a deliverable_ready mission outcome"
                )
            ...
            if item.lane != "lane-1":
                ...
                return "lane-1 integration accept needs next_mission when no research mission is paused"
        return None
    ...
    def _enqueue(
        ...
        item = {
            ...
            "deliverable": _copy(outcome["deliverable"]),
            "artifacts": _copy(outcome["artifacts"]),
    ...
    def invalidate_integration(self, item_id: str, reason: str) -> None:
        ...
        if source in LANES:
            self.trigger(
                ...
                scope="targeted",
                targets=[cast("LaneName", source)],
            )


__all__ = ["MISSION_PROTOCOL", "MissionController"]
